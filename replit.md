...
*   **Task Management:** Admin-published tasks are displayed to users, with real-time synchronization. An "Add Task" button is exclusively available and functional for Expert tier users. **Dynamic Tier-Based Pricing:** Task rewards are automatically calculated based on user's tier cap divided by total published tasks. Formula: `Reward = Tier Cap / Total Tasks`. If admin publishes 50 tasks: Basic users see £13/task (£650/50), Professional see £30/task (£1500/50), Expert see £60/task (£3000/50). Completing all tasks equals the tier's monthly earning cap.
...
*   **Team Meeting Room:** A WhatsApp-like chat interface displaying 3,031 total members with varying online status (2,100-2,800 users online). Uses a single global room where ALL users see the same messages - creating a shared community experience. Features a pinned admin message at the top: "Please mention only upgrades and withdrawals with status. Thank you." AI users (Grok-powered) with randomized UK and Indian names report to admin about their withdrawal successes (£1,400-£3,000 with "status complete") and tier upgrades (recharged £250 for T2 or £600 for T3). Each message uses a different random name. Messages use first person ("I have withdrawn...", "I upgraded...") and end with "thank you" or "thank you admin". Examples: "I have upgraded to tier 3, amount £600. Thank you." or "Withdrawal of £1,847 completed, thank you." Admin can enable/disable chat via Settings panel - when disabled, users can view messages but cannot send new ones. Messages continue in a loop as users join and interact.
